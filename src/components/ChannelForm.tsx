...
function parseChannelJson(
  ...
  for (let i = 0; i < o.playlists.length; i++) {
    ...
    if (!validation.valid) {
      return {
        error: `playlists[${i}]: ${validation.reason || 'Invalid URI'}`,
      };
    }
  }
...
export default function ChannelForm({
    ...
    getChannel(editId)
      .then((ch) => {
        ...
        try {
          ...
          setJsonText(
            JSON.stringify(stripSignatureFields(chForJson as object), null, 2)
          );
        }
        setJsonMode('form');
      })
      .catch((e) => {
        if (!cancelled) {
          setLoadError(
            e instanceof Error ? e.message : 'Failed to load channel'
          );
          loadedRef.current = null;
        }
      })
  ...
  const handleValidateURIs = () => {
    ...
    toast({
      ...
      description: `Checked ${uris.length} URIs`,
    });
  };

  const buildChannel = useCallback(
    (overridePlaylists?: string[]): Channel => {
  ...
  }, [jsonMode, isLoadingDoc, address, isEdit, serializeChannelJsonPreview]);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (overwriteBase && targetId) {
        if (
          !isWalletAuthorizedToOverwrite(overwriteBase, 'publisher', walletDID)
        ) {
          toast({
            title: 'Update failed',
      ...
      if (isEdit && editId) {
        const updated = await replaceChannel(
          editId,
          body,
              ...
              {availablePlaylists.length > 0 ? (
                <div className="mb-6 rounded-xl border border-border/60 bg-muted/20 p-4">
                  ...
                  <p className="mb-3 text-xs text-muted-foreground">
                    Click to add to this channel's{' '}
                    <code className="font-mono">playlists</code> list. Works
                    whether you're on the Form or JSON tab.
                  </p>
                ...
                <TabsContent value="form" className="mt-8 space-y-10">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid gap-4">
                      ...
                      <div>
                        <Label htmlFor="channel-coverImage">
                          Cover Image URL
                        </Label>
                        <Input
