...
describe('preparePlaylistGroupForPublish — create', () => {
  it('injects wallet curator when missing and wire matches signed bytes', () => {
    const r = preparePlaylistGroupForPublish({
      rawDocument: { ...baseGroup, curator: undefined },
      walletDID: WALLET,
    })
    ok<PlaylistGroup>(r)
    expect(r.signedPayload.curator).toBe(WALLET)
    expect(r.wireBody).toEqual(r.signedBytes)
    expect(r.toasts).toHaveLength(1)
    expect(r.toasts[0].title).toMatch(/curator/i)
  })

  it('replaces did:key curator with wallet; wire matches signed bytes', () => {
    const r = preparePlaylistGroupForPublish({
      rawDocument: { ...baseGroup, curator: DID_KEY },
      walletDID: WALLET,
    })
    ok<PlaylistGroup>(r)
    expect(r.signedPayload.curator).toBe(WALLET)
  ...
  it('CREATE with no slug — auto-generated slug appears identically in signed and wire', () => {
    const r = preparePlaylistGroupForPublish({
      rawDocument: {
        ...baseGroup,
        slug: undefined,
...
describe('preparePlaylistGroupForPublish — edit (slug drift regression)', () => {
  it('repaired curator and auto slug flow into BOTH signed payload and wire body', () => {
    const existing: PlaylistGroup = {
      ...baseGroup,
      id: 'grp-existing',
      created: '2026-05-20T00:00:00Z',
      slug: 'season-1-grp-exist',
      curator: DID_KEY,
    }
    const r = preparePlaylistGroupForPublish({
      rawDocument: {
    ...
    ok<PlaylistGroup>(r)
    expect(r.signedPayload.curator).toBe(WALLET)
    expect(r.wireBody.curator).toEqual(r.signedBytes.curator)
    expect(r.wireBody.slug).toEqual(r.signedBytes.slug)
    const { id: _id, created: _c, ...signedMinusOmissions } = r.signedBytes
    expect(r.wireBody).toEqual(signedMinusOmissions)
  })
  ...
  it('EDIT wireBody equals signedBytes minus id+created (only documented omissions)', () => {
    ...
    const r = preparePlaylistGroupForPublish({
      rawDocument: { ...baseGroup, title: 'edited' },
      walletDID: WALLET,
      base: existing,
    })
    ok<PlaylistGroup>(r)
