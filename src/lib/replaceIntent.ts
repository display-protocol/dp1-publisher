...
export async function buildReplaceIntent({
  ...
  const unsigned = {
    ...
    target: { type, id, slug },
    payloadHash: await payloadHashString(document),
    created: new Date().toISOString(),
  }

  // Signed over the intent with `signatures` stripped — the same rule the document itself follows, which
  // is what lets the feed verify both halves with one code path.
